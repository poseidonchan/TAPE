    ...
    if adaptive is True:
        if mode == 'high-resolution':
            ...
            for i in tqdm(range(len(test_x))):
                ...
                optimizerD = torch.optim.Adam(decoder_parameters, lr=1e-4, weight_decay=1e-6)
                optimizerE = torch.optim.Adam(encoder_parameters, lr=1e-4, weight_decay=1e-6)
            ...
            decoder_parameters = [{'params': [p for n, p in model.named_parameters() if 'decoder' in n]}]
            encoder_parameters = [{'params': [p for n, p in model.named_parameters() if 'encoder' in n]}]
            optimizerD = torch.optim.Adam(decoder_parameters, lr=1e-4, weight_decay=5e-6)
            optimizerE = torch.optim.Adam(encoder_parameters, lr=1e-4, weight_decay=5e-6)
