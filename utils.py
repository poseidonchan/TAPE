...
def counts2FPKM(counts,genelen='./Data/GeneLength.txt'):
...
def counts2TPM(counts,genelen='./Data/GeneLength.txt'):
...
def ProcessInputData(train_data, test_data, sep=None, datatype='TPM', variance_threshold=0.5):
    ...
    if datatype == 'FPKM':
        print('Transform to FPKM')
        train_x = counts2FPKM(train_x)
    elif datatype == 'TPM':
        print('Transform to TPM')
        train_x = counts2TPM(train_x)
